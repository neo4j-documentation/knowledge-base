...
[caption="Figure 1: "]
image::{assets-cdn}/memory_consumption_neo4j.png[Memory consumption in Neo4j,75%,75%]
...
Well - as we've seen above - the JVM will require some extra memory to function correctly.
For example, running a highly concurrent environment means that the memory occupied by the thread stack will be equal to the number of concurrent threads the JVM will process times the thread stack size (-Xss).

There are harder to find sources of non-heap memory use, such as buffer pools.
...
Usually, the `jcmd` dump is only moderately useful by itself.
It’s more common to take multiple dumps and compare them by running `jcmd <PID> VM.native_memory summary.diff`
